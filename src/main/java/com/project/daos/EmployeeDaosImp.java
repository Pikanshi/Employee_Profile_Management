...
public class EmployeeDaosImp implements CRUDRepository {
	...
	public boolean addEmployee(Employee employee) {
		...
		try(PreparedStatement pstmt = con.prepareStatement(sql)){
			...
			pstmt.setDate(9,Date.valueOf(employee.getBirthDate()));
			pstmt.setDate(10,Date.valueOf(employee.getWeddingAnniversary()));
	...
	public List<Employee> getEmployee(String name) throws NameNotFoundException {
		...
		try(PreparedStatement pstmt = con.prepareStatement(sql)) {
			...
			while(rs.next()) {
				...
				LocalDate birthDate= rs.getDate(9).toLocalDate();
				LocalDate weddingAnniversary= rs.getDate(10).toLocalDate();
				
				myEmployeeList.add(new Employee(id, firstName, lastName, dept, jobTitle, address, emailAddress, phoneNumber, birthDate, weddingAnniversary));
	...
	public Employee getEmployeeByID(int id) throws EmployeeNotFoundException   {
		...
		try(PreparedStatement pstmt = con.prepareStatement(sql)){
			...
			if(rs.next()) {
			   ...
			   String emailAddress = rs.getString(7);
			   String phoneNumber = rs.getString(8);
			   LocalDate birthDay = rs.getDate(9).toLocalDate();
			   LocalDate weddingAnniversary = rs.getDate(10).toLocalDate();
			   
			   myEmployee= new Employee(id,firstName,lastName,dept,jobTitle,address,emailAddress,phoneNumber,birthDay,weddingAnniversary);
			}
		}
		catch(Exception e) {
			e.printStackTrace();
	...
	public boolean editEmployee(Employee employee) {
		...
		int res=0;
		String sql = "update employee set firstName=?,lastName=?,dept=?,jobTitle=?,address=?,emailAddress=?,phoneNumber=?,birthDate=?,weddingAnniversary=? where Id = ?";
		...
		try(PreparedStatement pstmt = con.prepareStatement(sql)){
			...
			pstmt.setDate(8,Date.valueOf(employee.getBirthDate()));
			pstmt.setDate(9,Date.valueOf(employee.getWeddingAnniversary()));
	...
	public List<Employee> getEmployeeOfGivenBirthDate(LocalDate date) {
		...
		try(PreparedStatement pstmt = con.prepareStatement(sql)){
			...
			while(rs.next()) {
				...
				mylist.add(new Employee(id,firstName,lastName,dept,jobTitle,address,emailAddress,phoneNumber,birthDate,weddingAnniversary));
			}
		}
		catch(Exception e) {
			e.printStackTrace();
	...
	public List<Employee> getAllEmployees() {
		...
		try(PreparedStatement pstmt = con.prepareStatement(sql)){
			...
			while(rs.next()) {
				...
				mylist.add(new Employee(id,firstName,lastName,dept,jobTitle,address,emailAddress,phoneNumber,birthDay,weddingAnniversary));
			}
		}
		catch(Exception e) {
			e.printStackTrace();
